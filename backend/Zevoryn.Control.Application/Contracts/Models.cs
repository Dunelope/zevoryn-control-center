...
public sealed record CreateBetaCampaignRequest(Guid ProductId, string Name, string? Description, int MaxInvitations, DateTime? StartsAtUtc, DateTime? EndsAtUtc);
public sealed record UpdateBetaCampaignRequest(string Name, string? Description, int MaxInvitations, DateTime? StartsAtUtc, DateTime? EndsAtUtc);
public sealed record BetaCampaignDto(Guid Id, Guid ProductId, string Name, string? Description, BetaCampaignStatus Status, int MaxInvitations, int TotalInvitations, int PendingInvitations, int SentInvitations, int AcceptedInvitations, int RevokedInvitations, int FailedInvitations, int RemainingCapacity, DateTime? StartsAtUtc, DateTime? EndsAtUtc, DateTime CreatedAtUtc, DateTime UpdatedAtUtc);
public sealed record CreateBetaInvitationRequest(string Email, DateTime? ExpiresAtUtc);
public sealed record BetaInvitationDto(Guid Id, Guid BetaCampaignId, string Email, BetaInvitationStatus Status, string? ExternalReference, DateTime? InvitedAtUtc, DateTime? AcceptedAtUtc, DateTime? RevokedAtUtc, DateTime? ExpiresAtUtc, DateTime CreatedAtUtc, DateTime UpdatedAtUtc);
